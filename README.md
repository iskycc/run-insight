...
OpenLDAP 通常配置 `(uid={{username}})` 和 `entryUUID`；Active Directory 通常配置
`(&(objectClass=user)(sAMAccountName={{username}}))` 和 `objectGUID`。
`{{username}}` 是必需占位符，应用会按 RFC 4515 转义后再代入。服务账号只需搜索
用户条目的最小权限；找到唯一 DN 后，应用会使用该 DN 和用户提交的密码再次 bind。

绑定密码使用 AES-256-GCM 加密后保存，随机 256 位 AES 密钥也持久化在数据库中。
密码、密文和 AES 密钥不会通过 API、管理页面或审计日志返回。由于密钥与密文位于
同一数据库，此设计用于避免明文保存，不具备外部 KMS 对数据库完整泄露的隔离能力。
...
质量检查与 Docker 镜像构建是两个相互独立的并行 Job。`main` 分支或版本标签推送后，
镜像构建不等待质量检查完成；部署或采用新镜像前，必须确认同一提交对应的质量 Job
已经通过。CI 使用临时 MariaDB 服务、独立种子密码和占位 `JWT_SECRET`，不会连接
生产数据库。
